...
struct LassoOptions <: SolverOptions
  maxIter::Int64
  maxInnerIter::Int64
  optTol::Float64
end
...
LassoOptions(;
  ...
  optTol::Float64=1e-7) = LassoOptions(maxIter, maxInnerIter, optTol)
  ...
  function minimize_active_set1!{T<:AbstractFloat}(
    β::SparseVector{T},
    XX::StridedMatrix{T},
    Xy::StridedVector{T},
    λ::StridedVector{T},
    ...
    for iter=1:maxInnerIter
      ...
      for j = 1:length(rowval)
        ci = rowval[j]
        # Compute the Shoot and Update the variable
        S0 = compute_residual1(XX, Xy, β, ci) - XX[ci,ci]*nzval[j]
        oldVal = nzval[j]
        nzval[j] = shrink(-S0 / XX[ci,ci], λ[ci] / XX[ci,ci])
        if abs(oldVal - nzval[j]) > optTol
          fDone = false
  ...
  function compute_residual1{T<:AbstractFloat}(
    XX::StridedMatrix{T},
    Xy::StridedVector{T},
    β::SparseVector{T},
    k::Int64)
    return dot(XX[:, k], β) - Xy[k]
  end
  ...
  function add_violating_index1!{T<:AbstractFloat}(
    β::SparseVector{T},
    XX::StridedMatrix{T},
    Xy::StridedVector{T},
    λ::StridedVector{T})
    ...
    ind = 0
    for j = setdiff(1:p, rowval)
      S0 = abs(compute_residual1(XX, Xy, β, j))
      if S0 > λ[j]
        if S0 > val
          val = S0;
          ind = j;
...
lasso_raw!{T<:AbstractFloat}(
  ...
  y::StridedVector{T},
  λ::T,
  options=LassoOptions()) = lasso_raw!(X, y, λ*ones(size(X,2)), options)
...
function lasso_raw{T<:AbstractFloat}(
  ...
  lasso!(spzeros(p), XX, Xy, λ, options=LassoOptions())
end
...
lasso!{T<:AbstractFloat}(
  ...
  Xy::StridedVector{T},
  λ::T,
  options=LassoOptions()) = lasso!(β, XX, Xy, λ*ones(size(X,2)), options)
...
function lasso1!{T<:AbstractFloat}(
  β::SparseVector{T},
  XX::StridedMatrix{T},
  Xy::StridedVector{T},
  λ::Array{Float64, 1},
  ...
  if iszero(β)
    add_violating_index1!(β, XX, Xy, λ) != 0 || return β
  end

  for iter=1:options.maxIter
    minimize_active_set1!(β, XX, Xy, λ, options)
    add_violating_index1!(β, XX, Xy, λ) != 0 || return β
  end
  β
end
