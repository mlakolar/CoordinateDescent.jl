...
facts("cd sqrt-lasso") do
  ...
  for i=1:100
    ...
    x1 = coordinateDescent(f, g, CDOptions(;maxIter=5000, optTol=1e-8))
...
facts("scaled lasso") do
  ...
  for i=1:100
    ...
    x1 = coordinateDescent(f, g, CDOptions(;maxIter=5000, optTol=1e-8))
    x2, σh = scaledLasso(X, Y, λ, ScaledLassoOptions(;maxIter=100, optTol=1e-12), CDOptions(;maxIter=5000, optTol=1e-8))

    @fact maximum(abs.(x1 - x2)) --> roughly(0.; atol=2e-2)
  end
